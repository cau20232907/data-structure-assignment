...
void main() {
	...
	double insertionSortRec, quickSortRec, heapSortRec;
	double innerSortc;
	...
	endc = clock();
	dictionarySortCheck(tempArray, dictionary.numOfWords);
	insertionSortRec = (double)(endc - startc) / CLOCKS_PER_SEC;
	...
	endc = clock();
	dictionarySortCheck(tempArray, dictionary.numOfWords);
	quickSortRec = (double)(endc - startc) / CLOCKS_PER_SEC;
	...
	endc = clock();
	dictionarySortCheck(tempArray, dictionary.numOfWords);
	heapSortRec = (double)(endc - startc) / CLOCKS_PER_SEC;

	//내장정렬(3번)
	memcpy(tempArray, dictionary.dictionary, sizeof(Word) * dictionary.numOfWords);
	startc = clock();
	qsort(tempArray, dictionary.numOfWords, sizeof(Word), dictionaryCompareFunctionForInnersort);
	endc = clock();
	dictionarySortCheck(tempArray, dictionary.numOfWords);
	innerSortc = (double)(endc - startc) / CLOCKS_PER_SEC;


	printf("첫 정렬\n삽입 정렬: %.4f초\n퀵 정렬: %.4f초\n힙 정렬: %.4f초\n\n", insertionSortc, quickSortc, heapSortc);
	printf("재정렬\n삽입 정렬: %.4f초\n퀵 정렬: %.4f초\n힙 정렬: %.4f초\n\n", insertionSortRec, quickSortRec, heapSortRec);
	printf("내부 정렬: %.4f초", innerSortc);
