...
sortedDictionary = dictionary.copy()
startc = time.time()
sortedDictionary = sorted(sortedDictionary)
endc = time.time()
listSortedCheck(sortedDictionary)
innersort1Time = endc - startc
...
endc = time.time()
listSortedCheck(sortedDictionary)
innersort2Time = endc - startc

import numpy

sortedDictionary = dictionary.copy()
sortedDictionary = numpy.array(sortedDictionary)
startc = time.time()
sortedDictionary = numpy.sort(sortedDictionary)
endc = time.time()
sortedDictionary = sortedDictionary.tolist()
listSortedCheck(sortedDictionary)
numpysortTime = endc - startc

import pandas

sortedDictionary = dictionary.copy()
sortedDictionary = pandas.Series(sortedDictionary)
startc = time.time()
sortedDictionary.sort_values(inplace=True, kind="quicksort")
endc = time.time()
sortedDictionary = sortedDictionary.to_list()
listSortedCheck(sortedDictionary)
pandassortTime = endc - startc

print(f"삽입 정렬: {insertionsortTime}초")
print(f"퀵 정렬: {quicksortTime}초")
print(f"힙 정렬: {heapsortTime}초")

print(f"내부 정렬(sorted): {innersort1Time}초")
print(f"내부 정렬(sort): {innersort2Time}초")
print(f"numpy 정렬: {numpysortTime}초")
print(f"pandas 정렬: {pandassortTime}초")
